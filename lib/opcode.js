...
module.exports = exports = function(conn){
	...
	var handleData = function(callback){
		...
		for(var i=1;i<arguments.length;i++){
			var wrap = false;
			if(arguments[i] instanceof FlexibleBuffer){
				wrap = true;
	...
	var prefixAndExecute = function(arglist, func){
		...
		for(var i=0;i<arglist.length;i++){
			if(arglist[i] instanceof Buffer || arglist[i] instanceof FlexibleBuffer){
				if(i > 0 && typeof(arglist[i-1]) == "number"){
					var opcode = newArgList.pop();
					var buf = arglist[i];
					var wrap = false;
					if(buf instanceof FlexibleBuffer){
						wrap = true;
